...
**路由(`app/routes/`)。** `app-layout.tsx` 是受会话守护的外壳,顶部导航 此刻(`/`,`home.tsx`)→ 痕迹(`/timeline`)→ 照见(`/review`)→ 探寻(`/ask`)→ 设置(`/settings`),并挂载全局快速捕获 `QuickCapture`(⌘/Ctrl+J)。其余路由:`entry.tsx`(详情/编辑 `/entries/:id`)、`new.tsx`、`notes.tsx`、`calendar.tsx`、`capture.tsx`、`login.tsx`/`logout.tsx`、`upload.tsx`(附件上传)、`attachment.tsx`(解密读取),以及 `download-backup.tsx` 与 `api.*` 端点(`api.sync`、`api.entry-insight`、`api.summary`、`api.ask-stream`、`api.ask-stop`)。
