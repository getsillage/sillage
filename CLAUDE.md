...
单用户个人日记,完全运行在 Cloudflare Workers 上。Worker 上采用 React Router v8 框架模式(SSR),搭配 D1(SQL)、R2(附件)、KV(会话)。存储在服务端,带静态加密(非端到端加密)——服务端读取明文,以便运行全文搜索与 AI 摘要。界面文案为中文。
...
**搜索(`app/lib/search/`)。** `fts.ts` 实现 D1 FTS5 关键词搜索。
...
**备份(`app/lib/backup/export.ts`)。** 每日 cron(wrangler.jsonc 的 `triggers.crons`)将整个日记导出为 JSON + Markdown 到 R2 的 `backups/<date>/diary-<timestamp>.{json,md}`。`runScheduledBackup` 记录失败并重新抛出,使失败的 cron 运行可见。
