...
- Kotlin + Jetpack Compose Android 初版：`android/`
...
- Android 初版放在同仓库 `android/` 下，共用根目录 `proto/`；当前阶段为原生 Kotlin + Jetpack Compose 在线优先客户端，范围限于服务器地址配置、初始化/登录、记录列表、新建/编辑/删除。
...
- `/api/v1/sync` 和 `/api/v1/sync:push` 面向 Android 后续离线同步阶段，支持 tombstone、mutation id 幂等、冲突返回和附件 metadata 同步。
...
- 当前 Android 初版在线优先，只做服务器地址配置、初始化/登录、记录列表、新建/编辑/删除。
- Android 初版走 REST v1 包装层，暂不从 `proto/` 生成 Android 客户端，不实现 Room/WorkManager 离线同步。
