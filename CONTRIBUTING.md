...
7. The workflow composes the checked release-note input with a digest-pinned install command and signed evidence. Do not edit published notes to describe artifacts that differ from the immutable tag. Prerelease tags do not move `latest` or the `major.minor` stable alias.
