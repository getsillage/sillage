...
当前仓库默认部署为 beta 通道，线上访问不需要登录。部署后访问 `https://<name>.<account>.workers.dev` 冒烟测试：确认界面显示「β版」→ 留下一个带图片的片段 → 在探寻页搜索 → 打开附件 URL 可正常查看。

若切换为正式私有部署，请先把 `wrangler.jsonc` 中 `APP_RELEASE_CHANNEL` 改为 `production` 或移除该变量，再按登录链路冒烟测试：登录 → 留下一条记录 → 退出登录 → 未登录访问受保护页面应重定向到登录。
