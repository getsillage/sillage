...
当前仓库默认部署为 beta 通道，线上访问不需要登录。部署后访问 `https://<name>.<account>.workers.dev` 冒烟测试：确认界面显示「β版」→ 保存一个带图片的短记录 → 在问答页搜索 → 打开附件 URL 可正常查看。
