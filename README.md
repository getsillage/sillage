...
[`web/src/types/proto/`](web/src/types/proto/)。当前 Connect v1 已注册 AuthService、MemoService、
AttachmentService、SettingsService、AskService 与 SyncService，路径形如 `/sillage.api.v1.MemoService/ListMemos` 和
`/sillage.api.v1.SyncService/PullSync`；REST v1 仍保留现有 `/api/v1/*` 路由，并和 Connect 入口共用服务层逻辑。
