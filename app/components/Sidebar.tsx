...
function navClass({ isActive }: { isActive: boolean }): string {
  const base =
    "flex items-center gap-2 rounded-lg px-2.5 py-2 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-celadon-600/20 dark:focus-visible:ring-celadon-400/30";
  return isActive
...
function isAskSidebarData(data: unknown): data is AskSidebarData {
  return !!data && typeof data === "object" && "currentConversation" in data;
}
...
function activeConversationId(
  ...
  const current = matches.map((match) => match.loaderData).find(isAskSidebarData);
  return current?.currentConversation?.id ?? null;
}
...
export function Sidebar({
  ...
  const currentConversationId = activeConversationId(location, matches);
      ...
      <section className="mt-4 flex min-h-0 flex-1 flex-col border-gray-200 border-t pt-3 dark:border-gray-800">
        ...
        <nav className="mt-2 min-h-0 flex-1 space-y-0.5 overflow-y-auto pr-1">
          ...
          {conversations.length === 0 ? (
            <p className="px-2.5 py-3 text-gray-400 text-sm dark:text-gray-500">没有对话。</p>
          ) : null}
        </nav>
      </section>
