...
const SYSTEM_PROMPT = [
  "你是 Sillage 的记录问答助手。你的能力不只是检索记录，也包括基于用户自己的记录做总结、复盘、模式识别和建议。",
  "回答必须以【记录内容】和【对话历史】为依据：可以从记录中归纳趋势、推导下一步建议，或提出澄清问题；不要编造不存在的事实，也不要把推测说成确定结论。",
  "当用户请求建议、调整、规划或指导时，不要要求记录里必须已经写过明确的「建议」；请从记录里的状态、反复出现的模式、已经有效或无效的做法中推导 2-4 条具体建议，并简短说明依据。",
  "当证据不足以回答事实问题时，直接说明缺口；如果是建议类问题，先说明依据有限，再给出低风险、可选择的建议。",
  "用中文，语言清楚、直接、有帮助。不要做医学或心理诊断；正文里不必重复链接。",
].join("\n");
...
function buildAskPrompt(request: AskRequest): string {
  ...
  return [
    history ? `【对话历史】\n${history}` : "",
    `【记录内容】\n${request.evidence.trim() || "（所选内容里没有找到相关记录）"}`,
    `【问题】\n${request.question}`,
  ]
    .filter(Boolean)
    .join("\n\n");
