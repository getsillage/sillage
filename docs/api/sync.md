...
context 持久化已生成的部分回答。`parentId` / `forkOfId` 字段为未来的跟随分支与重新生成预留。

`askMessages.sourceRefs` 是结构化数组，至少包含 `memoId`、`entryDate`、`excerpt` 和 `rank`，供 Web 和未来 Android 客户端跳回来源 memo。流式与单轮路径都写入同一 `askConversations` / `askMessages` sync stream。
