...
当前 Connect v1 已注册 `AuthService`、`MemoService`、`SettingsService` 与 `SyncService`。REST v1 与 Connect
v1 的鉴权、AI 设置、memo 创建/更新/删除、AI 总结、sync pull 和 sync push 共用同一 service 逻辑；Ask
等 proto 已生成，运行时接入会随后续里程碑继续补齐。
