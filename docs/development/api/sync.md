...
Android 初版也走 REST v1 包装层，并在 `android/app/src/main/java/com/miofelix/sillage/data/SillageApi.kt` 维护接口调用与 JSON 解析。当前 Android 初版只覆盖服务器地址配置、初始化/登录、记录列表、新建/编辑/删除；不接入 `/api/v1/sync`，也不提交 Android proto 生成物。离线写入、Room 本地镜像、WorkManager 后台同步和从根目录 `proto/` 生成 Android 客户端属于后续阶段。
...
- Android 后续离线同步阶段可预生成 memo UUIDv7，并通过 `sync:push` 创建。当前 Android 初版在线创建 memo，使用普通 `POST /api/v1/memos`。
...
sync payload，也不会修改 `memo.updatedAt` 或 `version`。`query` 为空时返回按时间倒序的最近记录。

## Memo 动作接口（REST）

除创建 / 更新 / 删除外，memo 的状态变更和总结生成走以下动作端点（均带 `expectedVersion`）：
...
附件字节不进入 sync payload。Android 后续同步流程是先上传附件字节，再在 sync payload 中同步附件 metadata 与 memo 引用。当前 Android 初版不实现附件上传或展示。
...
每个 profile 有 `autoSummary` 开关：开启后，新建记录会在后台尽力生成单条总结（带超时、不阻塞写入、
失败仅告警）。
...
`askMessages.sourceRefs` 是结构化数组，至少包含 `memoId`、`entryDate`、`excerpt` 和 `rank`，供 Web 和后续 Android Ask 界面跳回来源 memo。流式与单轮路径都写入同一 `askConversations` / `askMessages` sync stream。当前 Android 初版不实现 Ask。
