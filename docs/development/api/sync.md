...
Android 初版也走 REST v1 包装层，并在 `android/app/src/main/java/app/sillage/data/SillageApi.kt` 维护接口调用与 JSON 解析。当前 Android 初版只覆盖服务器地址配置、初始化/登录、记录列表、新建/编辑/删除；不接入 `/api/v1/sync`，也不提交 Android proto 生成物。离线写入、Room 本地镜像、WorkManager 后台同步和从根目录 `proto/` 生成 Android 客户端属于后续阶段。
