...
cross-holder list-surface, interactive-workspace, and canonical-memo transitions.
Android's root UI state now owns one `records` aggregate value with transitional
slice getters for the former top-level holder fields. Coordinated list-surface,
workspace teardown, and canonical memo writes go through the aggregate; request
identity and late-response checks stay on the individual holders.
