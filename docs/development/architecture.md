...
核心 REST 与 Connect 操作复用 `server/api_service.go` 中的业务方法。附件上传、Ask SSE 等手写扩展会在 route 中编排并直接调用 Store；它们仍须复用相同的鉴权和领域约束，不能另造冲突语义。
...
| `server/` | HTTP/Connect 适配、认证、业务服务和 AI 调用 |
