...
draft/Markdown updates, attachment-upload request transitions, source-record
absorption, and canonical memo writes go through the aggregate; request identity
and late-response checks stay on the individual holders. Android request helpers
update that aggregate through `withRecords`.
