...
- 「开始问答」按钮 → `/ask` 并 `startNew()`。
- 主导航 `NavLink`：写记录 `/`（书写类图标）、全部记录 `/timeline`（记录集合类图标）。不使用容易暗示浏览器历史的 `History` 图标；选中态 = 灰色实底，闲置态 = muted + hover。
- 「问答」会话区（ChatGPT 式 `conversations`）默认显示未归档会话，active 高亮当前会话；标题栏提供服务端搜索和「已归档问答」入口，每行可直接归档或恢复。搜索 / 视图切换会隔离迟到响应，归档当前会话后回到空白问答。
