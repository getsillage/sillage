...
replace, interactive workspace teardown, detail/editor presentation, and
canonical memo application so list loads, search ownership, and selection stay
consistent. Android's root `SillageUiState` now stores one `records` aggregate
field and keeps transitional slice getters (`recordsCollection`,
`recordsPagination`, and the other former top-level fields) for existing call
sites. Coordinated host writes such as cache mutation, visible-list
clear/replace/append, workspace teardown, selected-memo presentation, editor
session starts, and filter reset go through the aggregate; individual holders
still own request identity.
