...
class SearchRecordsUseCaseTest {
    @Test
    fun forwardsSemanticSearchAndReturnsDomainRecords() {
        val expectedQuery = RecordsSearchQuery(
            text = "shared state",
            scope = RecordsQueryScope.Archived,
        )
        val expected = listOf(memo("memo-1"))
        var capturedQuery: RecordsSearchQuery? = null
        val useCase = SearchRecordsUseCase(
            repository = object : RecordsSearchRepository {
