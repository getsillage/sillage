...
- `RecordsFeatureStateHolder` composes the holders above and owns coordinated
  ...
  append), browse filter/view-mode application, interactive workspace teardown,
  detail/editor presentation (present/clear/save/return selected memo, begin
  editor drafts, absorb a source memo into cache/search, and complete detail
  with canonical cache application), and canonical memo application (cache
