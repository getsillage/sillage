...
func TestAISettingsEncryptKeyAndHidePlaintext(t *testing.T) {
	...
	if strings.Contains(res.Body.String(), "sk-secret") || !strings.Contains(res.Body.String(), `"hasApiKey":true`) {
...
func TestMemoSummaryAndKeyUnavailable(t *testing.T) {
	...
	if res.Code != http.StatusOK || !strings.Contains(res.Body.String(), `"summary"`) {
	...
	if res.Code != http.StatusOK || !strings.Contains(res.Body.String(), `"memoAi"`) || !strings.Contains(res.Body.String(), `"local-summary"`) {
